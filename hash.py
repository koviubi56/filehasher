...
def main(
    batch_mode: Dict[
        Union[Literal["enabled", "file"]], Union[bool, str]
    ] = {"enabled": False}
):
    """
    The main function
            ...
            window = sg.Window(
                ...
                [
                    ...
                    [
                        sg.Text("SHA3 512: "),
                        sg.InputText(
                            default_text=sha3_512.hexdigest(),
                            disabled=True,
                        ),
                    ],
                    [
                        sg.Text("Blake2s: "),
                        sg.InputText(
                            default_text=blake2s.hexdigest(),
                            disabled=True,
                        ),
                    ],
                    [
                    ...
                    [
                        sg.Text("SHA3 256: "),
                        sg.InputText(
                            default_text=sha3_256.hexdigest(),
                            disabled=True,
                        ),
                    ],
                    [
                        sg.Text("SHA3 224: "),
                        sg.InputText(
                            default_text=sha3_224.hexdigest(),
                            disabled=True,
                        ),
                    ],
                    [
                        sg.Text("Blake2b: "),
                        sg.InputText(
                            default_text=blake2b.hexdigest(),
                            disabled=True,
                        ),
                    ],
                    [
                        sg.Text("SHA3 384: "),
                        sg.InputText(
                            default_text=sha3_384.hexdigest(),
                            disabled=True,
                        ),
                    ],
                    [
                    ...
                    [
                        ...
                        sg.Button(
                            "Check hash from file", key="-IMPORT-"
                        ),
                    ],
                ],
            )
            ...
            if event == "-IMPORT-":
                try:
                    with open(f"{file}.hash") as f:
                        fc = load(f)
                        if (
                            sha512.hexdigest().lower()
                            == fc["sha512"].lower()
                            and sha256.hexdigest().lower()
                            == fc["sha256"].lower()
                        ):
                            sg.Popup(
                                "Matches!",
                            ...
                            sg.Popup(
                                "Does not match! You may got hacked."
                            )
                except FileNotFoundError:
